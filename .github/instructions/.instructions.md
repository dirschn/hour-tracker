...
1. **User** - System users
   ...
   - Associations: has_many employments, positions (through employments), shifts
...
3. **Position** - Job positions within companies
   ...
   - Associations: belongs_to company, has_many employments, users (through employments)

4. **UserPosition** - Junction table linking users to positions
   - Fields: start_date, end_date
   - Associations: belongs_to user, belongs_to position, has_many shifts, has_many hourly_rates

5. **Shift** - Individual time tracking records
   - Fields: minutes_worked, date, description, notes (rich text)
   - Associations: belongs_to employment, belongs_to user, belongs_to group (optional)
   - Validations: minutes_worked > 0 and <= 1440, unique user per position per date

6. **Group** - User-defined categories for organizing shifts
   - Fields: name, description, color
   - Associations: belongs_to user, has_many shifts

7. **HourlyRate** - Historical rate tracking for employments
   - Fields: rate_cents, effective_date
   - Associations: belongs_to employment
...
- Shifts are validated to prevent duplicates per user/position/date
- Rich text support is available for shift notes
- Groups provide color-coded organization for shifts
